...
@ApplyOptions<PaginatedMessageCommand.Options>({
	...
	subCommands: [
		{ input: 'film', output: StarWars.Resource.Films },
		StarWars.Resource.Films,
		{ input: 'character', output: StarWars.Resource.People },
		{ input: 'person', output: StarWars.Resource.People },
		{ input: StarWars.Resource.People, default: true },
		{ input: 'planet', output: StarWars.Resource.Planets },
		StarWars.Resource.Planets
	]
})
export class UserCommand extends PaginatedMessageCommand {
	public async [StarWars.Resource.People](message: GuildMessage, args: PaginatedMessageCommand.Args) {
		const { t } = args;
		const [query, loadingMessage] = await Promise.all([args.rest('string'), sendLoadingMessage(message, t)]);

		const peopleData = await this.fetchApi<StarWars.Resource.People>({ resource: StarWars.Resource.People, query });
		...
		for (const result of peopleData.results) {
			display.addAsyncPageEmbed(async (embed) => {
				const personHomeworld = await this.fetchApi<StarWars.Resource.Planets, StarWars.DetailOrSearch.Detail, false>({
				...
				for (const species of result.species) {
					const speciesData = await this.fetchApi<StarWars.Resource.Species, StarWars.DetailOrSearch.Detail, false>({
						url: species,
						throwOnError: false
					});
				...
				for (const starship of result.starships) {
					const starshipData = await this.fetchApi<StarWars.Resource.Starships, StarWars.DetailOrSearch.Detail, false>({
						url: starship,
						throwOnError: false
					});
				...
				for (const vehicle of result.vehicles) {
					const vehicleData = await this.fetchApi<StarWars.Resource.Vehicles, StarWars.DetailOrSearch.Detail, false>({
						url: vehicle,
						throwOnError: false
					});
				...
				for (const film of result.films) {
					const filmData = await this.fetchApi<StarWars.Resource.Films, StarWars.DetailOrSearch.Detail, false>({
						url: film,
						throwOnError: false
					});
	...
	public async [StarWars.Resource.Films](message: GuildMessage, args: PaginatedMessageCommand.Args) {
		const { t } = args;
		const [query, loadingMessage] = await Promise.all([args.rest('string'), sendLoadingMessage(message, t)]);

		const filmsData = await this.fetchApi<StarWars.Resource.Films>({ resource: StarWars.Resource.Films, query });
			...
			display
				...
				.addAsyncPageEmbed(async (embed) => {
					...
					for (const character of result.characters.slice(0, 3)) {
						const characterData = await this.fetchApi<StarWars.Resource.People, StarWars.DetailOrSearch.Detail, false>({
							url: character,
							throwOnError: false
						});
					...
					for (const planet of result.planets.slice(0, 3)) {
						const planetsData = await this.fetchApi<StarWars.Resource.Planets, StarWars.DetailOrSearch.Detail, false>({
							url: planet,
							throwOnError: false
						});
	...
	public async [StarWars.Resource.Planets](message: GuildMessage, args: PaginatedMessageCommand.Args) {
		const { t } = args;
		const [query, loadingMessage] = await Promise.all([args.rest('string'), sendLoadingMessage(message, t)]);

		const planetData = await this.fetchApi<StarWars.Resource.Planets>({ resource: StarWars.Resource.Planets, query });
